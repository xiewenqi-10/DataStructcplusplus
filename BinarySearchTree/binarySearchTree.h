 ...
 * @LastEditTime: 2020-09-01 02:45:41
...
public:
    ...
    TreeNode*Max(TreeNode*root,vector<int>&v);
};
...
void binarySearchTree::insert(TreeNode*root,int val)
{
    if(root->val>val)                   //若根结点>val
    {
        if(root->left==nullptr) root->left=new TreeNode(val); //若左子树为空时，左子树=TreeNode(val);
        else insert(root->left,val);    //否则就插手左子树
    }
    else
    {
        if(root->right==nullptr) root->right=new TreeNode(val);//若右子树为空时，右子树=TreeNode(val);
        else insert(root->right,val);   //否则就插手左子树
    }  
}
